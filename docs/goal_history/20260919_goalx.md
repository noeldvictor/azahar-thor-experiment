...
This is a copy of the goal as it stood on 2026-09-19 (now `docs/goal.md`), kept so the goals we set
and what they produced can be reviewed later. The live goal is `docs/goal.md`; `/goal` is Claude Code's built-in command and takes a
completion condition that points at it.
