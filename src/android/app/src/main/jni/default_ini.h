...
# Measurement switch. Skips the image barrier at the end of every render pass to show what
# those barriers cost. Not correct: a later draw that samples the pass output may read
# stale pixels. Leave it off unless you are taking that measurement.
# 0 (default): Off, 1: On
)") DECLARE_KEY(skip_pass_barriers) BOOST_HANA_STRING(R"(

# Samples each texture unit once per fragment instead of once per reference. Several
# combiner stages ask for the same unit, and each ask used to fetch it again. Same image.
# 0: Off, 1 (default): On
)") DECLARE_KEY(cache_texture_samples) BOOST_HANA_STRING(R"(

# Measurement switch. Ends and restarts the render pass every N draws so the cost of one
# render pass can be read off the frame time. The picture stays correct.
